...
### 阶段一实际跑出来的结果(2026-08-20,3000 prompt × 20 采样,共 59,980 次生成)

```
mean = 474, median = 268
p90 = 1255, p95 = 1769, p99 = 2048(顶格)
max = 2048
恰好等于 2048(撞上限)的样本:2326 / 59980 ≈ 3.9%
```

**过程中发现并修复的两个问题**(详见对应的 git commit):
1. 一开始用 `llm.generate()` 直接喂原始文本(没套用聊天模板),Qwen3 默认开着的"思考模式"导致几乎所有生成都顶到 2048 上限——不是真实的回复长度分布,是没套用聊天模板的假象。改用 `llm.chat()` + `chat_template_kwargs={"enable_thinking": False}` 修复。
2. `fit_logt` 在某个 prompt 的 20 次采样长度完全相同(零方差)时,L-BFGS-B 会在这种退化的似然面上不收敛、直接返回没动过的初始猜测(`sigma=1.0`)——加了一个兜底:检测到零方差直接返回一个很小的固定 `sigma`,不跑优化器。

**修复之后,~3.9% 的样本仍然会撞到 2048 上限**——确认过这不是 bug:均值(474)远大于中位数(268)说明右偏分布确实存在,只是长尾被 `max_tokens=2048` 这道硬截断削平了(接近 2000 的样本里几乎全部精确卡在 2048,是右截断的典型信号,不是自然停止)。**决定:保留 2048 这个上限不重新跑**,理由是这个值和 TIE 论文自己 `ua_score_calculator.py` 里用的截断常数一致,论文的打分方法本身就假设/接受这种截断;当作已知的右截断处理,在最终报告里明确记录这 ~4% 的比例作为已知限制,不额外花 GPU 时间重新生成。

### 数据规模:决定扩到 10,000(2026-08-20)

一开始考虑用 4 个并行 SLURM job 冲论文的 45,000 规模,但发现结果合并脚本有个不好处理的坑(合并结果如果又被当成下一次合并的输入,shard 数据会被重复计入两次——即"合并操作不是幂等的"),加上 4 个 job 共享同一个 vLLM `torch.compile` 缓存还有并发写冲突的已知问题([vLLM #24601](https://github.com/vllm-project/vllm/issues/24601))。综合考虑工程复杂度,**放弃并行方案,改为单个 job 顺序跑**:已有的 3,000 个(index 0-2999)之外,再跑 7,000 个新的(`--start-index 3000 --num-prompts 7000`,输出到 `data/qwen3_8b_logt_labels_extra.csv`/`qwen3_8b_length_samples_extra.csv`,跑完手动 `cat` 合并成最终的 `data/qwen3_8b_logt_labels.csv`),**总共凑到 10,000 个 prompt**。

**这 10,000 个 prompt 具体是哪些,是完全确定性、可复现的**:`stream_first_turn_prompts(num_prompts=10000, start_index=0)`——即 lmsys-chat-1m 数据集流式读取的**第 0 条到第 9999 条**(不是随机抽样),只要数据集本身不变,同样参数任何时候重跑都会拿到完全相同的这 10,000 条,顺序也一样。原文本身也直接存在 `data/qwen3_8b_logt_labels.csv`/`qwen3_8b_length_samples.csv` 里,不依赖"记住"这件事,只要这两个文件被提交进 git 就不会丢。

**⚠️ 阶段三重要约束(为了后续 SSJF/TIE 对比实验的公平性)**:这 10,000 个 prompt(index 0-9999)是用来训练/验证预测器的,**阶段三 benchmark 的真实请求 workload 必须避开这个范围**,应该用 **index ≥ 10,000** 之后的 prompt。原因:如果 TIE/SSJF 这类依赖预测器的调度算法在 benchmark 阶段服务的正是预测器训练时见过的同一批 prompt,预测器等于是"背答案",和不依赖预测器的 FCFS/Oracle-SJF 对比就不公平(经典的训练/测试数据泄漏)。

**数据实际到手(2026-08-21)**:两批(pilot 2999 + 续跑的 6999,中途因为 HPC 那份代码没 `git pull` 到最新版重跑过一次、又碰上疑似共享节点内存挤兑被杀过一次,详见对话记录)合并成 9998 个 prompt,已提交进 `data/qwen3_8b_length_samples.csv`(原始采样)和 `data/qwen3_8b_logt_labels.csv`(拟合结果,直接从合并后的采样数据重新拟合,不依赖任何中间过程文件)。

**独立验证:log-t vs log-normal 的 KS 拟合优度对比**(`scripts/ks_pass_rate.py`,纯本地计算,不需要 GPU):对全部 9591 个有效 prompt(407 个 20 次采样长度完全相同的退化案例被排除,不计入统计)逐一做 KS 检验:

| | log-t | log-normal | 论文(n=100/prompt) |
|---|---|---|---|
| 通过率(p>0.05) | **80.8%** | 75.7% | log-t 93.1% / log-normal 60.3% |

log-t 通过率始终更高,方向和论文一致;差距(5.1 个百分点)比论文小很多(32.8 个百分点),这是**预期之内**的——我们每个 prompt 只有 20 次采样(论文 100 次),KS 检验统计把握度更低,两边通过率都被"拉高"、差距被压缩,不代表我们的 log-t 优势比论文弱。用 2999 和 9591 两个不同规模的子集分别算过,结果几乎一致(81.9%/76.4% vs 80.8%/75.7%),说明这个结论是稳定的,不是小样本噪声。
